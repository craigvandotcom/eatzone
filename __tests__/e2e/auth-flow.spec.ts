...
test.describe('Authentication Flow', () => {
  ...
  test('signup flow handles both new user creation and existing user scenarios', async ({
    page,
  }) => {
    ...
    if (currentUrl.includes('/app')) {
      ...
      await expect(page.getByText(/body compass/i).first()).toBeVisible();
      console.log(
        '✓ Signup successful - user authenticated and redirected to dashboard'
      );
      ...
      const hasSuccessMessage = await page
        .getByText(/check your email|verification|success/i)
        .first()
        .isVisible()
        .catch(() => false);
