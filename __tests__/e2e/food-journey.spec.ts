...
test.describe('Food Tracking Journey', () => {
  ...
  test('food capture interface loads and displays camera controls', async ({ page }) => {
    ...
    if (cameraFound) {
      ...
      console.log('📝 Camera interface not immediately visible (may require interaction)');
    }
  ...
  test('manual food entry form works correctly', async ({ page }) => {
    ...
    if (manualEntryFound && activeElement) {
      ...
      const ingredientsField = page.getByRole('textbox', { name: /ingredient/i }).first();
      if (await ingredientsField.isVisible().catch(() => false)) {
        await ingredientsField.fill('spinach, tomatoes, olive oil, feta cheese');
      }
      
      // Look for save/submit button
      const saveButton = page.getByRole('button', { name: /save|add|submit/i }).first();
  ...
  test('ingredient zoning colors are displayed correctly', async ({ page }) => {
    ...
    const zoneElements = await page.locator('*[class*="zone"], *[class*="green"], *[class*="yellow"], *[class*="red"]').all();
    ...
    if (zoneElements.length > 0) {
      ...
      const hasZoneStyles = await page.evaluate(() => {
        ...
        return !!(styles.getPropertyValue('--zone-green') || 
  ...
  test.skip('complete food tracking flow with authentication', async ({ page }) => {
    ...
    console.log('Full authenticated food tracking flow - placeholder for future implementation');
  });
});