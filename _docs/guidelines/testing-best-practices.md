...
* **Types + tests + tooling** all work together: TypeScript prevents whole classes of bugs *before* runtime, tests prove behavior *at* runtime, and tools enforce discipline *all the time*.
...
expectTypeOf(parseUser({ id: "1", name: "A" })).toEqualTypeOf<{ id: number; name: string }>();
...
* **Mocking**: default to **MSW** at the network layer. Only stub modules when the collaborator is *truly* slow or nondeterministic (like `crypto`, `Date`, or a UUID generator).
...
* **Flaky E2E** → Seed state via API, wait for *conditions* not *time*, and use robust role-based selectors.
