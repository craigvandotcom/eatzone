...
Sentry.init({
  ...
  beforeSend(event, hint) {
    ...
    // Remove sensitive data from local storage and indexedDB breadcrumbs
    if (event.breadcrumbs) {
      event.breadcrumbs = event.breadcrumbs.filter(breadcrumb => {
        // Filter out IndexedDB operations that might contain health data
        if (breadcrumb.category === "console" && breadcrumb.message) {
          return (
            !breadcrumb.message.includes("IndexedDB") ||
            !breadcrumb.message.includes("health") ||
            !breadcrumb.message.includes("symptom") ||
            !breadcrumb.message.includes("ingredient")
          );
        }
        return true;
      });
  ...
  // Configure integrations for better privacy
  integrations: [
    Sentry.breadcrumbsIntegration({
      // Don't capture console logs that might contain health data
